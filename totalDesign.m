...
% unusedRatio=2*1.1*totvolfrac; %when the stress in an element is lower than the mean stress divided by that factor, it is considered as unused.
% unusedRatio2=2*2*totvolfrac; %different factor used in a secound "stress filter"
% unusedRatio3=2*20*totvolfrac;
...
% totalMat(repelem(stressTotMat<meanSTM/unusedRatio,2,2))=0;
...
ittt=0;
totalMatInt=zeros(size(totalMat));
while ittt==0 || ((mean(mean(totalMatInt)) < 2*totvolfrac -1 && ittt<30))
    totalMatInt=totalMat;
    totalMatInt(repelem(stressTotMat<meanSTM/unusedRatio,2,2))=0; %eliminate "unused" elements from the total design matrix
    unusedRatio=2*unusedRatio;
    unusedRatio2=2*unusedRatio2;
    unusedRatio3=2*unusedRatio3;
    ittt=ittt+1;
end
totalMat=totalMatInt;
